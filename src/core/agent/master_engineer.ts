...
export class MasterEngineer extends BaseAgent {
  constructor() {
    super(
      'Master Engineer',
      'Senior Code Engineer',
      `You are the Master Engineer, responsible for implementing all code in TITAN++.
...
Generate complete, working code that follows industry best practices.`
  ...
  private async generateCode(task: AgentTask): Promise<AgentResponse> {
...
Output only the code, properly formatted.`;
  ...
  private async implementFeature(task: AgentTask): Promise<AgentResponse> {
...
Provide the complete implementation with all necessary changes.`;
  ...
  private async createModule(task: AgentTask): Promise<AgentResponse> {
...
Provide complete, production-ready code.`;
  ...
  private async buildAPI(task: AgentTask): Promise<AgentResponse> {
...
Provide complete, working API code.`;
  ...
  private async handleGeneric(task: AgentTask): Promise<AgentResponse> {
    const result = await this.think(
      `Implement this: ${JSON.stringify(task.payload)}`
    );
