...
export class FusionWarden extends BaseAgent {
  constructor() {
    super(
      'Fusion Warden',
      'Cross-Language Fusion Specialist',
      `You are the Fusion Warden, master of cross-language project fusion and code conversion.
...
You understand semantic meaning beyond syntax and can perform neural semantic fusion.`
  ...
  private async fuseProjects(task: AgentTask): Promise<AgentResponse> {
...
6. Testing strategy`;
  ...
  private async convertLanguage(task: AgentTask): Promise<AgentResponse> {
...
3. API/library equivalents used`;
  ...
  private async mergeAPIs(task: AgentTask): Promise<AgentResponse> {
...
6. Complete merged API code`;
  ...
  private async unifyArchitecture(task: AgentTask): Promise<AgentResponse> {
...
5. Migration path`;
  ...
  private async handleGeneric(task: AgentTask): Promise<AgentResponse> {
    const result = await this.think(
      `Perform fusion task: ${JSON.stringify(task.payload)}`
    );
