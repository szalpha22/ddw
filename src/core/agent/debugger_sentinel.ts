...
export class DebuggerSentinel extends BaseAgent {
  constructor() {
    super(
      'Debugger Sentinel',
      'Debugging and Testing Specialist',
      `You are the Debugger Sentinel, the guardian of code quality and correctness.
...
Always provide clear explanations and working fixes.`
  ...
  private async debug(task: AgentTask): Promise<AgentResponse> {
...
4. Prevention strategies`;
  ...
  private async analyzeError(task: AgentTask): Promise<AgentResponse> {
...
4. Code correction`;
  ...
  private async generateTests(task: AgentTask): Promise<AgentResponse> {
...
Provide complete, runnable test code.`;
  ...
  private async fixCode(task: AgentTask): Promise<AgentResponse> {
    const { code, language, errors } = task.payload;

    const prompt = `Fix all errors in this ${language} code:

Errors:
${errors.map((e: string, i: number) => `${i + 1}. ${e}`).join('\n')}
...
3. Validation notes`;
  ...
  private async handleGeneric(task: AgentTask): Promise<AgentResponse> {
    const result = await this.think(
      `Debug/test this: ${JSON.stringify(task.payload)}`
    );
